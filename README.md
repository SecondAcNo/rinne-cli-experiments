...
- ~~**compress**：既存スナップショットの圧縮（実験的）~~（deep archiveへの集約を確定）
- **deep archive**：チャンク共有＋圧縮による長期保管（実験的）
- **内部構造の刷新**：将来の拡張に備えた再編（実験的）
- **コマンドの刷新**：将来の拡張に備えた再編（実験的）
- **言語変更の検討** : 速度が求められる部分についてC++を検討
- **除外コマンドの検討** : textdiffやdiffなど将来的に無用となりうる機能の削減検討
- **GUI・AI・マルチプラットフォーム化への布石** : v0.9.2で予定。v0.9.1ではCLIの更新のみ
