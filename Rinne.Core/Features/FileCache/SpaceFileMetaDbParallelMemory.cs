...
public sealed class SpaceFileMetaDbParallelMemory : IDisposable
{
    private readonly string _dbPath;
    private readonly string _connectionString;
    private bool _disposed;

    private readonly Dictionary<string, SpaceFileMeta> _cache;
    ...
    private SpaceFileMetaDbParallelMemory(string dbPath, string connectionString, Dictionary<string, SpaceFileMeta> cache)
    {
        _dbPath = dbPath;
    ...
    public static SpaceFileMetaDbParallelMemory Open(string dbPath)
    {
        ...
        var cache = new Dictionary<string, SpaceFileMeta>(StringComparer.Ordinal);
        using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = @"
                SELECT path, size, mtime_ticks, file_hash, chunk_hashes
                FROM filemeta;";
            ...
            while (reader.Read())
            {
                ...
                var fileHash = reader.GetString(3);
                var chunkJson = reader.GetString(4);
                ...
                cache[path] = new SpaceFileMeta(size, mtime, fileHash, chunks);
            }
        }

        return new SpaceFileMetaDbParallelMemory(dbPath, cs, cache);
    }
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(nameof(SpaceFileMetaDbParallelMemory));
    }
    ...
    public SpaceFileMeta? TryGet(string relativePath)
    {
        ...
        if (_cache.TryGetValue(relativePath, out var meta))
        {
            return meta;
        }
