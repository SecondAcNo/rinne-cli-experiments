...
public sealed class CompactParallelService
{
    private readonly RinnePaths _paths;

    public CompactParallelService(RinnePaths paths) => _paths = paths;
    ...
    public async Task<Summary> RunAsync(string space, IReadOnlyList<SnapshotInfo> targets, CompactOptions opt, CancellationToken ct)
    {
        ...
        using (var fileMetaDb = SpaceFileMetaDbParallel.Open(fileMetaDbPath))
        {
            foreach (var s in targets)
            {
                ...
                try
                {
                    ...
                    await SaveDirectoryOnePassParallelPipe.RunAsync(
                        inputDir: payloadDir,
                        storeDir: storeDir,
