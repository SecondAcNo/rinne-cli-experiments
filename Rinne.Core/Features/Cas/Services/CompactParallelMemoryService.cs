...
public sealed class CompactParallelMemoryService
{
    private readonly RinnePaths _paths;

    public CompactParallelMemoryService(RinnePaths paths) => _paths = paths;
    ...
    public async Task<Summary> RunAsync(string space, IReadOnlyList<SnapshotInfo> targets, CompactOptions opt, CancellationToken ct)
    {
        ...
        // ★ ここだけ SpaceFileMetaDbParallel → SpaceFileMetaDbParallelMemory
        using (var fileMetaDb = SpaceFileMetaDbParallelMemory.Open(fileMetaDbPath))
        {
            foreach (var s in targets)
            {
                ...
                try
                {
                    ...
                    await SaveDirectoryOnePassParallelMemoryPipe.RunAsync(
                        inputDir: payloadDir,
                        storeDir: storeDir,
